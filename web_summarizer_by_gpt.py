...
mcp = FastMCP("web_summarizer_by_gpt")
...
# 用 gpt 模型對一段文字做總結
def summarize_with_gpt(text: str,model = "gpt-4",temperature = 0.3) -> str:
...
def summarize_text(text: str, user_prompt: str) -> str:
    ...
    if text_tokens >= 3000:
        ...
        for chunk in chunks:
            ...
            summary = summarize_with_gpt(text = summary_prompt, model = "gpt-4", temperature = 0.3)
        ...
        final_summary = summarize_with_gpt(text = final_prompt)
        ...
        return summarize_with_gpt(summary_prompt)

@mcp.tool()
async def summarize_web_by_gpt(url: str, instruction: str) -> str:
    """
    Fetch a webpage and use the gpt model to summarize it based on user instruction.
